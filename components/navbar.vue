<template>
  <nav>
    ...
    <ul class="menu" :class="{ open: first, close: !drawer }">
      <li
        v-for="(x, i) in navs"
        :key="x.title + i"
        class="menu-link"
        :class="{ sub: x.mini.length > 0, active: x.clicked }"
        @click.stop="close(x)"
      >
        <nuxt-link v-if="x.mini.length === 0" :to="x.link" class="text-item"
          >{{ x.title }}<span v-if="x.mini.length > 0" class="icon"></span
        ></nuxt-link>
        <div v-else :to="x.link" class="text-item">
          {{ x.title }}<span v-if="x.mini.length > 0" class="icon"></span>
        </div>
        <ul v-if="x.mini.length > 0" class="submenu">
          <li
            v-for="m in x.mini"
            :key="m.title"
            class="sub-item"
            @click.stop="subclose(x)"
          >
            <nuxt-link class="text-white" :to="m.link">{{ m.title }}</nuxt-link>
          </li>
        </ul>
      </li>
      <!-- <li class="menu-link"><a href="#0" class="text-item">Home</a></li>
...
export default {
  props: {
    drawer: { default: true, type: Boolean },
  },
  data() {
    return {
      first: false,
      navs: [
        { title: 'Home', mini: [], link: '/' },
        {
          title: 'Portfolio',
          link: '/portfolio',
          mini: [
        ...
        { link: '/history', title: 'History', mini: [] },
        { title: 'Misc', link: '/', mini: [] },
      ],
    }
  },
  watch: {
